...
#     realizado_count     = Diario.objects.filter(realizado=realizado)[0]
#     descartado_count    = Diario.objects.filter(descartado=desacartado)[0]
#     confirmado_count    = Diario.objects.filter(confirmado=confirmado)[0]
#     recuperado_count    = Diario.objects.filter(recuperado=recuperado)[0]    
#     obito_count         = Diario.objects.filter(obito=obito)[0]
#     isolado_count       = Diario.objects.filter(isolado=isolado)[0]
#     leito_count         = Diario.objects.filter(leito=leito)[0]
#     uti_count           = Diario.objects.filter(uti=uti)[0]
#     criacao = timezone.now()

#     context = {
#         'cidade':prefeitura,
#         'realizado':realizado_count,
